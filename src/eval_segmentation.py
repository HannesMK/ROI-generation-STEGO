...
def my_app(cfg: DictConfig) -> None:
    ...
    result_dir = os.path.join(cfg.output_root, "results", "predictions", cfg.experiment_name)
    ...
    for model_path in cfg.model_paths:
        ...
        if cfg.evaluate_all_frames:
            image_set = "all"
        else:
            image_set = "val"

        if cfg.res is None:
            resolution = model.cfg.res
        else:
            resolution = cfg.res
        ...
        test_dataset = ContrastiveSegDataset(
            ...
            crop_type=None,
            image_set=image_set,
            transform=get_transform(resolution, False, loader_crop),
            target_transform=get_transform(resolution, True, loader_crop),
            cfg=model.cfg,
        )
        ...
        if model.cfg.dataset_name == "cocostuff27":
            ...
            if cfg.evaluate_all_frames:
                all_good_images = range(len(test_dataset))
            else:
                all_good_images = range(10)
        else:
            ...
            for i, batch in enumerate(tqdm(test_loader)):
                with torch.no_grad():
                    ...
                    if cfg.run_crf:
                        ...
                        if torch.cuda.is_available():
                            linear_preds = linear_preds.cuda()
                            cluster_preds = cluster_preds.cuda()
                    else:
                    ...
                    if i in batch_nums:
                        for offset in range(len(linear_preds.cpu())):
        ...
        for good_images in batch_list(range(len(all_good_images)), 10):
            ...
            fig, ax = plt.subplots(n_rows, len(good_images), figsize=(len(good_images) * 3, n_rows * 3), squeeze=False)
            ...
            for i, img_num in enumerate(good_images):
                ...
                ax[index_row, i].imshow(plot_img)
                index_row += 1
                
                if model.cfg.has_labels:
                    ax[index_row, i].imshow(plot_label)
                    index_row += 1
                ...
                if cfg.run_prediction:
                    ...
                    ax[index_row, i].imshow(plot_cluster)
                    index_row += 1

                if run_picie:
                    picie_img = model.label_cmap[saved_data["picie_preds"][img_num]].astype(np.uint8)
                    ax[index_row, i].imshow(picie_img)
                    index_row += 1
                    Image.fromarray(picie_img).save(join(join(result_dir, "picie", str(img_num) + ".png")))
            
            if not cfg.show_images:
                plt.close()
                continue

            index_row = 0

            ax[index_row, 0].set_ylabel("Image", fontsize=26)
            index_row += 1

            if model.cfg.has_labels:
                ax[index_row, 0].set_ylabel("Label", fontsize=26)
                index_row += 1

            if cfg.run_prediction:
                ax[index_row, 0].set_ylabel("STEGO\n(Ours)", fontsize=26)
                index_row += 1

            if run_picie:
                ax[index_row, 0].set_ylabel("PiCIE\n(Baseline)", fontsize=26)
                index_row += 1
        ...
        if model.cfg.dataset_name == "directory":
            continue
